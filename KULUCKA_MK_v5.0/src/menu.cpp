...
MenuManager::MenuManager() {
    ...
    _menuChanged = true;
    _menuOffset = 0;  // **EKLEME: Kaydırma offset'i**
...
void MenuManager::update(JoystickDirection direction) {
    ...
    if (_isTerminalMenu(_currentState)) {
        if (direction == JOYSTICK_LEFT) {
            ...
            _selectedIndex = 0;
            _menuOffset = 0;  // **EKLEME: Offset sıfırla**
            _menuChanged = true;
    ...
    switch (direction) {
        case JOYSTICK_UP:
            ...
            _updateMenuOffset(); // **EKLEME: Kaydırma kontrolü**
            _menuChanged = true;
            break;
        ...
        case JOYSTICK_RIGHT:
            if (_selectedIndex < currentItems.size()) {
                ...
                if (nextState != MENU_NONE) {
                    ...
                    _selectedIndex = 0;
                    _menuOffset = 0;  // **EKLEME: Yeni menüde offset sıfırla**
                    _menuChanged = true;
...
// **EKLEME: Menü offset değerini al**
...
bool MenuManager::selectMenuItem(int index) {
    ...
    if (index >= 0 && index < currentItems.size()) {
        ...
        _updateMenuOffset(); // **EKLEME: Offset güncelle**
        _menuChanged = true;
        return true;
    }
...
void MenuManager::setCurrentState(MenuState state) {
    ...
    _currentState = state;
    _selectedIndex = 0;
    _menuOffset = 0;  // **EKLEME: Yeni state'te offset sıfırla**
    _menuChanged = true;
    updateInteractionTime();
}
