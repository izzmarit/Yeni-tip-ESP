...
void MenuManager::_handleTimeAdjustment(JoystickDirection direction) {
    switch (direction) {
        ...
        case JOYSTICK_LEFT:
            if (_timeField == 0) {
                // Saat alanındayken sol tuş = geri çık
                _currentState = _previousState;
                _menuChanged = true;
                Serial.println("Saat ayarlama ekranından geri dönüldü");
            } else {
                // Dakika alanından saat alanına geç
                _timeField = 0;
                Serial.println("Saat alanına geçildi");
            }
            break;
            
        case JOYSTICK_RIGHT:
            if (_timeField == 0) {
                // Saat alanından dakika alanına geç
                _timeField = 1;
                Serial.println("Dakika alanına geçildi");
            } else {
                // Dakika alanındayken sağ tuş = kaydet (JOYSTICK_PRESS olarak işle)
                // Bu işlem main.cpp'de JOYSTICK_PRESS olarak işlenecek
                Serial.println("Saat kaydetme işlemi tetiklendi");
            }
            break;
...
void MenuManager::_handleDateAdjustment(JoystickDirection direction) {
    ...
    switch (direction) {
        ...
        case JOYSTICK_LEFT:
            if (_dateField == 0) {
                // Gün alanındayken sol tuş = geri çık
                _currentState = _previousState;
                _menuChanged = true;
                Serial.println("Tarih ayarlama ekranından geri dönüldü");
                return; // Return erken çık, _dateValue güncellemesi yapma
            } else {
                // Bir önceki alana geç
                _dateField = (_dateField - 1 + 3) % 3;
                Serial.println("Önceki tarih alanına geçildi: " + String(_dateField));
            }
            break;
            
        case JOYSTICK_RIGHT:
            if (_dateField == 2) {
                // Yıl alanındayken sağ tuş = kaydet (JOYSTICK_PRESS olarak işle)
                Serial.println("Tarih kaydetme işlemi tetiklendi");
            } else {
                // Bir sonraki alana geç
                _dateField = (_dateField + 1) % 3;
                Serial.println("Sonraki tarih alanına geçildi: " + String(_dateField));
            }
            break;
