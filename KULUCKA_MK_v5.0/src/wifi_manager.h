...
private:
    ...
    unsigned long _lastDiscoveryResponse;
    
    void _setupWiFiEventHandlers();

    void _setupMDNS();

    void _updateConnectedClients();
    void _handleStationDisconnect();
    void _handleStationConnect();
    void _startDiscoveryService();
    void _handleDiscoveryPackets();
    void _sendDiscoveryResponse(IPAddress remoteIP, uint16_t remotePort);
