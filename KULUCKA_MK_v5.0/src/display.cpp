...
void Display::_updateTempSection(float currentTemp, float targetTemp, bool heatingActive) {
    // Sıcaklık değeri, ısıtıcı durumu veya yanıp sönme zamanlaması
    ...
    if (heatingActive) {
        ...
        if (currentMillis - lastBlinkMillis >= 250) {
            shouldUpdate = true;
...
void Display::_updateHumidSection(float currentHumid, float targetHumid, bool humidActive) {
    // Nem değeri, nemlendirici durumu veya yanıp sönme zamanlaması
    static unsigned long lastBlinkMillis = 0;
    ...
    if (shouldUpdate) {
        ...
        if (humidActive) {
            // Yanıp sönme için 250ms interval ile saniyede 2 kez
            if ((millis() / 250) % 2 == 0) {
