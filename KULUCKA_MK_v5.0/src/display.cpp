...
void Display::_updateTempSection(float currentTemp, float targetTemp, bool heatingActive) {
    // Sıcaklık değeri veya ısıtıcı durumu değiştiyse güncelle
    if (abs(currentTemp - _lastTemp) >= 0.1 || targetTemp != _lastTargetTemp || heatingActive != _lastHeatingActive) {
        // Sıcaklık bölmesini temizle
        _tft.fillRect(1, 16, SCREEN_WIDTH / 2 - 1, (SCREEN_HEIGHT - 15) / 2 - 1, COLOR_BACKGROUND);
        
        _tft.setTextSize(1);
        
        // SICAKLIK başlığı (sadece ısıtıcı çalışıyorsa yanıp sönsün)
        if (heatingActive) {
            // millis() / 250 ile saniyede 2 kez yanıp sönme (4 durum, 2'si açık 2'si kapalı)
            if ((millis() / 250) % 4 < 2) {
...
void Display::_updateHumidSection(float currentHumid, float targetHumid, bool humidActive) {
    // Nem değeri veya nemlendirici durumu değiştiyse güncelle
    if (abs(currentHumid - _lastHumid) >= 0.5 || targetHumid != _lastTargetHumid || humidActive != _lastHumidActive) {
        // Nem bölmesini temizle
        _tft.fillRect(SCREEN_WIDTH / 2 + 1, 16, SCREEN_WIDTH / 2 - 1, (SCREEN_HEIGHT - 15) / 2 - 1, COLOR_BACKGROUND);
        
        _tft.setTextSize(1);
        
        // NEM başlığı (sadece nemlendirici çalışıyorsa yanıp sönsün)
        if (humidActive) {
            // millis() / 250 ile saniyede 2 kez yanıp sönme (4 durum, 2'si açık 2'si kapalı)
            if ((millis() / 250) % 4 < 2) {
...
void Display::_updateMotorSection(int minutesLeft, int secondsLeft, bool motorActive) {
    // Motor durumu değiştiyse güncelle
    if (minutesLeft != _lastMotorMin || secondsLeft != _lastMotorSec || motorActive != _lastMotorActive) {
        ...
        // Dakika ve saniye gösterimi - Metin boyutu 1.5 olarak ayarlandı (1 ve 2 arasında)
        _tft.setTextSize(1);  // Boyut 2'den 1.5'e (gerçekte 1'e) indirildi
        
        // Dakika gösterimi - büyük rakamlar
        _tft.setCursor(5, 90);
        _tft.print("Dk:");
        
        _tft.setTextSize(2);  // Dakika değeri büyük
        ...
        _tft.setTextSize(2);  // Saniye değeri büyük
        _tft.setCursor(25, 108);
        
        // Saniye sayacı - motor çalışmaya başladığında geri sayacak
        if (motorActive) {
            _tft.print(secondsLeft);
        } else {
            // Motor çalışmıyorken ayarlanan saniyeyi göster
            _tft.print(DEFAULT_MOTOR_RUN_TIME);
        }
...
void Display::showMenu(String menuItems[], int itemCount, int selectedItem) {
    // Ekran modu değişti - tekrar çizimi engellemek için
