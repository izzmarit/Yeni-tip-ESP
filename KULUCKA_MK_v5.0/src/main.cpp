...
void handleJoystick() {
    ...
    if (direction == JOYSTICK_LEFT) {
        ...
        // Tarih ayarlama için (handleJoystick fonksiyonu içinde)
...
void handleValueAdjustment(JoystickDirection direction) {
    if (direction == JOYSTICK_PRESS) {
        ...
        // Kayıt işlemi
        if (criticalUpdate || calibrationUpdate) {
            storage.saveStateNow();
            Serial.println("Veriler anında kaydedildi - Tip: " + String(calibrationUpdate ? "Kalibrasyon" : "Kritik"));
        } else {
            storage.queueSave();
        }
        ...
        // Bir üst menüye dönüş - DÜZELTME: Public metodu kullan
