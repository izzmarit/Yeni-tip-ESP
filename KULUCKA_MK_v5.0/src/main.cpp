...
void handleWifiParameterUpdate(String param, String value) {
    ...
    if (alarm >= 0.1 && alarm <= 5.0) {
        ...
        storage.queueSave();
        Serial.print("Düşük sıcaklık alarm eşiği güncellendi: ");
        Serial.println(alarm);
    } else {
        Serial.print("Geçersiz düşük sıcaklık alarm değeri: ");
        Serial.println(alarm);
    }
    ...
    if (alarm >= 0.1 && alarm <= 5.0) {
        ...
        storage.queueSave();
        Serial.print("Yüksek sıcaklık alarm eşiği güncellendi: ");
        Serial.println(alarm);
    } else {
        Serial.print("Geçersiz yüksek sıcaklık alarm değeri: ");
        Serial.println(alarm);
    }
    ...
    if (alarm >= 1 && alarm <= 20) {
        ...
        storage.queueSave();
        Serial.print("Düşük nem alarm eşiği güncellendi: ");
        Serial.println(alarm);
    } else {
        Serial.print("Geçersiz düşük nem alarm değeri: ");
        Serial.println(alarm);
    }
    ...
    if (alarm >= 1 && alarm <= 20) {
        ...
        storage.queueSave();
        Serial.print("Yüksek nem alarm eşiği güncellendi: ");
        Serial.println(alarm);
    } else {
        Serial.print("Geçersiz yüksek nem alarm değeri: ");
        Serial.println(alarm);
    }
}
    ...
    // Kuluçka ayarları
      ...
      if (type <= INCUBATION_MANUAL) {
         ...
         // PID kontrolcüsünün hedef değerini güncelle
         pidController.setSetpoint(newTargetTemp);
        
         // Hysteresis kontrolcüsünün hedef değerini güncelle
         hysteresisController.setSetpoint(newTargetHumid);
        
         storage.queueSave();
        
         Serial.println("Kuluçka tipi değiştirildi. Yeni hedef değerler:");
         Serial.print("Sıcaklık: ");
         Serial.println(newTargetTemp);
         Serial.print("Nem: ");
         Serial.println(newTargetHumid);

         // Manuel kuluçka tipiyse ek log
         if (type == INCUBATION_MANUAL) {
             Serial.println("Manuel kuluçka modu aktif. Parametreler manuel olarak ayarlanabilir.");
            }
     }
    ...
    // Manuel kuluçka parametreleri
      ...
      if (temp >= 20.0 && temp <= 40.0) {
          ...
          // Manuel kuluçka tipindeyse ve gelişim aşamasındaysa PID hedef değerini güncelle
          if (incubation.getIncubationType() == INCUBATION_MANUAL && incubation.getCurrentStage() == STAGE_DEVELOPMENT) {
              pidController.setSetpoint(temp);
              Serial.print("Manuel gelişim sıcaklığı güncellendi: ");
              Serial.println(temp);
          }
      ...
      if (temp >= 20.0 && temp <= 40.0) {
          ...
          // Manuel kuluçka tipindeyse ve çıkım aşamasındaysa PID hedef değerini güncelle
          if (incubation.getIncubationType() == INCUBATION_MANUAL && incubation.getCurrentStage() == STAGE_HATCHING) {
              pidController.setSetpoint(temp);
              Serial.print("Manuel çıkım sıcaklığı güncellendi: ");
              Serial.println(temp);
          }
      ...
      if (humid >= 30 && humid <= 90) {
          ...
          // Manuel kuluçka tipindeyse ve gelişim aşamasındaysa hysteresis hedef değerini güncelle
          if (incubation.getIncubationType() == INCUBATION_MANUAL && incubation.getCurrentStage() == STAGE_DEVELOPMENT) {
              hysteresisController.setSetpoint(humid);
              Serial.print("Manuel gelişim nemi güncellendi: ");
      ...
      if (humid >= 30 && humid <= 90) {
          ...
          // Manuel kuluçka tipindeyse ve çıkım aşamasındaysa hysteresis hedef değerini güncelle
          if (incubation.getIncubationType() == INCUBATION_MANUAL && incubation.getCurrentStage() == STAGE_HATCHING) {
              hysteresisController.setSetpoint(humid);
              Serial.print("Manuel çıkım nemi güncellendi: ");
              Serial.println(humid);
          }
