...
void performMotorTest() {
    ...
    while (millis() - startTime < testDurationMillis) {
        // Her 500ms'de bir watchdog besle ve durum güncelle
        if (millis() % 500 == 0) {
            watchdogManager.feed();
            
            // Kalan süreyi hesapla ve göster
            unsigned long remaining = (testDurationMillis - (millis() - startTime)) / 1000;
            ...
            display.showProgressBar(
                ...
                SCREEN_WIDTH - 40, 20, 
                COLOR_HIGHLIGHT, 
                100 - ((millis() - startTime) * 100 / testDurationMillis)
            );
        }
