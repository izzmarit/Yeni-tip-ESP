...
void handleMenuActions(JoystickDirection direction) {
    ...
    // KALİBRASYON AYARLARI - SENSÖR İNDEKSLERİ DÜZELTİLMİŞ
    if (currentState == MENU_CALIBRATION_TEMP_1) {
        if (direction == JOYSTICK_RIGHT || direction == JOYSTICK_PRESS) {
            menuManager.showValueAdjustScreen(
                "Sensor 1 Sicaklik Kal.", 
                sensors.getTemperatureCalibration(0), // Sensör 0'ın kalibrasyonu
                "C", 
    ...
    if (currentState == MENU_CALIBRATION_TEMP_2) {
        if (direction == JOYSTICK_RIGHT || direction == JOYSTICK_PRESS) {
            menuManager.showValueAdjustScreen(
                ...
                sensors.getTemperatureCalibration(1), // Sensör 1'in kalibrasyonu
                "C", 
                TEMP_CALIBRATION_MIN, 
                TEMP_CALIBRATION_MAX, 
                0.1
    ...
    if (currentState == MENU_CALIBRATION_HUMID_1) {
        if (direction == JOYSTICK_RIGHT || direction == JOYSTICK_PRESS) {
            menuManager.showValueAdjustScreen(
                "Sensor 1 Nem Kal.", 
                sensors.getHumidityCalibration(0), // Sensör 0'ın kalibrasyonu
                "%", 
    ...
    if (currentState == MENU_CALIBRATION_HUMID_2) {
        if (direction == JOYSTICK_RIGHT || direction == JOYSTICK_PRESS) {
            menuManager.showValueAdjustScreen(
                "Sensor 2 Nem Kal.", 
                sensors.getHumidityCalibration(1), // Sensör 1'in kalibrasyonu
                "%", 
                HUMID_CALIBRATION_MIN, 
                HUMID_CALIBRATION_MAX, 
                0.5
    ...
    if (direction == JOYSTICK_PRESS) {
        ...
        switch (prevState) {
            ...
            case MENU_CALIBRATION_TEMP_1:
                // DÜZELTME: Sadece sensör 0'ın kalibrasyonunu ayarla
            ...
            case MENU_CALIBRATION_TEMP_2:
                // DÜZELTME: Sadece sensör 1'in kalibrasyonunu ayarla
        ...
        if (prevState >= MENU_CALIBRATION_TEMP_1 && prevState <= MENU_CALIBRATION_TEMP_2) {
