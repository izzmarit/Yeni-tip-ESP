...
void loop() {
    ...
    // Periyodik otomatik kayıt (her 1 dakikada bir) - YENİ EKLENDİ
    static unsigned long lastPeriodicSave = 0;
    if (currentMillis - lastPeriodicSave >= 60000) { // 60 saniye
        ...
        if (storage.getPendingChanges() > 0) {
            ...
            watchdogManager.endOperation();
            Serial.println("Periyodik otomatik kayıt tamamlandı");
        }
    }
