...
void handleMenuActions(JoystickDirection direction) {
            ...
            case MENU_WIFI_MODE:
    {
        ...
        if (currentMode == WIFI_CONN_MODE_AP) {
            ...
            // Station SSID kontrolü yap
            String stationSSID = storage.getStationSSID();
            String stationPassword = storage.getStationPassword();
            
            Serial.println("Station SSID: '" + stationSSID + "' (Uzunluk: " + String(stationSSID.length()) + ")");
            Serial.println("Station Password uzunluğu: " + String(stationPassword.length()));
            ...
            // Station moduna geç
            Serial.println("Station moduna geçiş başlatılıyor...");
            ...
            Serial.println("AP moduna geçiş başlatılıyor...");
    ...
    break;
            ...
            case MENU_WIFI_SSID:
    {
        // SSID ayarlama işlemi - Telefon uygulamasından yapılacak
        String currentSSID = storage.getStationSSID();
        if (currentSSID.length() == 0) {
            display.showConfirmationMessage("SSID: Ayarlanmamis");
        } else {
            display.showConfirmationMessage("SSID: " + currentSSID);
        }
        display.showConfirmationMessage("Telefon Uygulamasindan");
        display.showConfirmationMessage("Ayarlayin");
        menuManager.setCurrentState(MENU_WIFI_SETTINGS);
    }
    break;
...
case MENU_WIFI_PASSWORD:
    {
        // Password ayarlama işlemi - Telefon uygulamasından yapılacak
        String currentPassword = storage.getStationPassword();
        if (currentPassword.length() == 0) {
            display.showConfirmationMessage("Sifre: Ayarlanmamis");
        } else {
            display.showConfirmationMessage("Sifre: ****");
        }
        display.showConfirmationMessage("Telefon Uygulamasindan");
        display.showConfirmationMessage("Ayarlayin");
        menuManager.setCurrentState(MENU_WIFI_SETTINGS);
            ...
            case MENU_INCUBATION_TYPE:
    {
        ...
        if (selectedType == 3) {
            ...
            storage.setPidMode(1);  // YENİ: Manuel PID modunu kaydet
            storage.queueSave();
...
// MENU_MANUAL_START case'ini de güncelleyin:
    ...
    break;
            ...
            case MENU_PID_AUTO_TUNE:
                 if (!pidController.isAutoTuneEnabled()) {
                     ...
                     storage.setPidMode(2);  // YENİ: Storage'a kaydet
                     storage.queueSave();    // YENİ: Kayıt kuyruğuna ekle
            ...
            case MENU_PID_OFF:
                ...
                storage.setPidMode(0);  // YENİ: Storage'a kaydet
                storage.queueSave();    // YENİ: Kayıt kuyruğuna ekle
            ...
            case MENU_PID_MANUAL_START:
                pidController.setPIDMode(PID_MODE_MANUAL);
                storage.setPidMode(1);  // YENİ: Storage'a kaydet
                storage.queueSave();    // YENİ: Kayıt kuyruğuna ekle
...
void handlePIDAutoTune() {
    ...
    if (pidController.isAutoTuneEnabled()) {
        ...
        if (pidController.isAutoTuneFinished()) {
            ...
            storage.setPidMode(1);  // YENİ: Otomatik ayarlama sonrası manuel moda geç
            storage.queueSave();
            updateMenuWithCurrentStatus();
        }
...
void handleWifiParameterUpdate(String param, String value) {
    ...
    if (param == "targetTemp") {
        ...
        if (temp >= 20.0 && temp <= 40.0) {
            ...
            storage.saveStateNow(); // KRİTİK: Anında kaydet
