...
void loop() {
    ...
    if (!menuManager.isInHomeScreen() && 
        ...
        display.setupMainScreen(); // Ana ekranı yeniden ayarla
    }
...
void handleJoystick() {
    ...
    if (direction != JOYSTICK_NONE) {
        ...
        // Ana ekranda sadece sağ yön ve buton basma kontrolü
        if (menuManager.isInHomeScreen()) {
            if (direction == JOYSTICK_RIGHT) {
                // Sadece sağ yön için menüye geç
                menuManager.updateInteractionTime();
                menuManager.update(direction);
                
                // Menü değişti, ekranı güncelle
                display.showMenu(
            ...
            // Ana ekranda diğer yönler için HİÇBİR ŞEY YAPMA
        } else {
            // Menüde ise normal joystick kontrollerini işle
            menuManager.updateInteractionTime();
            
            // Menü işlemlerini gerçekleştir
            menuManager.update(direction);
            
            // Menü durumunu kontrol et ve ekranı güncelle
            ...
            } else if (menuManager.getCurrentState() == MENU_NONE) {
        ...
        // Joystick reset ihtiyacını belirle
        joystickNeedsReset = true;
        joystickResetTime = millis();
    }
}
    ...
    if (direction == JOYSTICK_PRESS) {
        switch (currentState) {
            ...
            case MENU_ADJUST_VALUE:
                ...
                if (menuManager.getPreviousState() == MENU_TEMPERATURE) {
                    // Değişikliği uygula - sıcaklık için
                    float newTemp = menuManager.getAdjustedValue();
                    // TODO: İlgili işlemler yapılacak
                    
                    display.showConfirmationMessage("Deger kaydedildi");
                    storage.queueSave(); // Gecikmeli kayıt
                } else if (menuManager.getPreviousState() == MENU_HUMIDITY) {
                    // Değişikliği uygula - nem için
                    int newHumid = (int)menuManager.getAdjustedValue();
                    // TODO: İlgili işlemler yapılacak
...
void handleValueAdjustment(JoystickDirection direction) {
    ...
    // Sol yön tuşuna basılması değeri değiştirmek yerine menüye dönüş yapacak
    // Bu kontrol MenuManager::update içinde yapılıyor artık
    
    // Buton basıldığında değer değişimini kaydet
    if (direction == JOYSTICK_PRESS) {
        MenuState prevState = menuManager.getPreviousState();
        float adjustedValue = menuManager.getAdjustedValue();
        
        // İlgili modüle değeri kaydet
        switch (prevState) {
            case MENU_TEMPERATURE:
                // Sıcaklık değerini kaydet
                if (incubation.getCurrentStage() == STAGE_DEVELOPMENT) {
                    // Gelişim aşamasındaki sıcaklık değerini ayarla
                    ...
                    if (incubation.getIncubationType() == INCUBATION_MANUAL) {
                        incubation.setManualParameters(
                            adjustedValue,  // Yeni gelişim sıcaklığı
                            params.hatchingTemp,
                    ...
                    // Diğer modlar için gerekirse eklenecek
                } else if (incubation.getCurrentStage() == STAGE_HATCHING) {
                    // Çıkım aşamasındaki sıcaklık değerini ayarla
                    IncubationParameters params = incubation.getParameters();
                    if (incubation.getIncubationType() == INCUBATION_MANUAL) {
                        incubation.setManualParameters(
                            params.developmentTemp,
                            adjustedValue,  // Yeni çıkım sıcaklığı
                            params.developmentHumidity,
                            params.hatchingHumidity,
                            params.developmentDays,
                            params.hatchingDays
                ...
                // PID hedef değerini güncelle
                pidController.setSetpoint(adjustedValue);
                break;
                
            case MENU_HUMIDITY:
                // Nem değerini kaydet
                if (incubation.getCurrentStage() == STAGE_DEVELOPMENT) {
                    // Gelişim aşamasındaki nem değerini ayarla
                    IncubationParameters params = incubation.getParameters();
                    if (incubation.getIncubationType() == INCUBATION_MANUAL) {
                        incubation.setManualParameters(
                            params.developmentTemp,
                            params.hatchingTemp,
                            (uint8_t)adjustedValue,  // Yeni gelişim nemi
                            params.hatchingHumidity,
                            params.developmentDays,
                            params.hatchingDays
                        );
                    ...
                    // Çıkım aşamasındaki nem değerini ayarla
                    IncubationParameters params = incubation.getParameters();
                    if (incubation.getIncubationType() == INCUBATION_MANUAL) {
                        incubation.setManualParameters(
                            params.developmentTemp,
                            params.hatchingTemp,
                            params.developmentHumidity,
                            (uint8_t)adjustedValue,  // Yeni çıkım nemi
                            params.developmentDays,
                            params.hatchingDays
                        );
                    }
                ...
                // Histerezis hedef değerini güncelle
                hysteresisController.setSetpoint(adjustedValue);
                break;
                
            case MENU_PID:
                // PID değerlerini kaydet
                if (menuManager.getSelectedIndex() == 0) {
                    pidController.setTunings(adjustedValue, 
                                             pidController.getKi(), 
                                             pidController.getKd());
                    storage.setPidKp(adjustedValue);
                } else if (menuManager.getSelectedIndex() == 1) {
                    pidController.setTunings(pidController.getKp(), 
                                             adjustedValue, 
                                             pidController.getKd());
                    storage.setPidKi(adjustedValue);
                } else if (menuManager.getSelectedIndex() == 2) {
                    pidController.setTunings(pidController.getKp(), 
                                             pidController.getKi(), 
                                             adjustedValue);
                    storage.setPidKd(adjustedValue);
                }
                break;
                
            case MENU_MOTOR_WAIT:
                // Motor bekleme süresini kaydet
                relays.updateMotorTiming(millis(), 
                                        (uint32_t)adjustedValue,
                                        relays.getMotorRunTimeLeft());
                storage.setMotorWaitTime((uint32_t)adjustedValue);
                break;
                
            case MENU_MOTOR_RUN:
                // Motor çalışma süresini kaydet
                relays.updateMotorTiming(millis(),
                                        relays.getMotorWaitTimeLeft(),
                                        (uint32_t)adjustedValue);
                storage.setMotorRunTime((uint32_t)adjustedValue);
                break;
                
            case MENU_SET_TIME:
                // Saat ayarını kaydet
                {
                    int timeValue = (int)adjustedValue;
            ...
            case MENU_SET_DATE:
                // Tarih ayarını kaydet
                {
                    long dateValue = (long)adjustedValue;
            ...
            case MENU_CALIBRATION_TEMP:
                // Sıcaklık kalibrasyon değerini kaydet
                sensors.setTemperatureCalibration(
                    adjustedValue,
                    sensors.getTemperatureCalibration(1)
                );
                storage.setTempCalibration(0, adjustedValue);
                break;
                
            case MENU_CALIBRATION_HUMID:
                // Nem kalibrasyon değerini kaydet
                sensors.setHumidityCalibration(
                    adjustedValue,
                    sensors.getHumidityCalibration(1)
                );
                storage.setHumidCalibration(0, adjustedValue);
                break;
                
            case MENU_ALARM_TEMP_LOW:
                // Düşük sıcaklık alarm eşiğini kaydet
                alarmManager.setTempLowThreshold(adjustedValue);
                storage.setTempLowAlarm(adjustedValue);
                break;
                
            case MENU_ALARM_TEMP_HIGH:
                // Yüksek sıcaklık alarm eşiğini kaydet
                alarmManager.setTempHighThreshold(adjustedValue);
                storage.setTempHighAlarm(adjustedValue);
                break;
                
            case MENU_ALARM_HUMID_LOW:
                // Düşük nem alarm eşiğini kaydet
                alarmManager.setHumidLowThreshold(adjustedValue);
                storage.setHumidLowAlarm(adjustedValue);
                break;
                
            case MENU_ALARM_HUMID_HIGH:
                // Yüksek nem alarm eşiğini kaydet
                alarmManager.setHumidHighThreshold(adjustedValue);
                storage.setHumidHighAlarm(adjustedValue);
                break;
            ...
            case MENU_MANUAL_HATCH_DAYS:
                // Manuel kuluçka parametrelerini kaydet
                {
                    IncubationParameters params = incubation.getParameters();
                    float devTemp = params.developmentTemp;
                    float hatchTemp = params.hatchingTemp;
                    uint8_t devHumid = params.developmentHumidity;
                    uint8_t hatchHumid = params.hatchingHumidity;
                    uint8_t devDays = params.developmentDays;
                    uint8_t hatchDays = params.hatchingDays;
                    
                    // İlgili parametreyi güncelle
                    switch (prevState) {
                        case MENU_MANUAL_DEV_TEMP:
                            devTemp = adjustedValue;
                            break;
                        case MENU_MANUAL_HATCH_TEMP:
                            hatchTemp = adjustedValue;
                            break;
                        case MENU_MANUAL_DEV_HUMID:
                            devHumid = (uint8_t)adjustedValue;
                            break;
                        case MENU_MANUAL_HATCH_HUMID:
                            hatchHumid = (uint8_t)adjustedValue;
                            break;
                        case MENU_MANUAL_DEV_DAYS:
                            devDays = (uint8_t)adjustedValue;
                            break;
                        case MENU_MANUAL_HATCH_DAYS:
                            hatchDays = (uint8_t)adjustedValue;
                            break;
                        default:
                            break;
                    }
                    
                    // Güncellenen parametrelerle manuel kuluçka ayarlarını güncelle
                    incubation.setManualParameters(
                        devTemp,
                        hatchTemp,
                        devHumid,
                    ...
                    // Storage'a kaydet
                    storage.setManualDevTemp(devTemp);
                    storage.setManualHatchTemp(hatchTemp);
                    storage.setManualDevHumid(devHumid);
                    storage.setManualHatchHumid(hatchHumid);
                    storage.setManualDevDays(devDays);
                    storage.setManualHatchDays(hatchDays);
                }
                break;
        ...
        // Değer ayarlama ekranından çıkış - ekstra menü güncellemesi
        display.showMenu(
            menuManager.getMenuItems().data(),
            menuManager.getMenuItems().size(),
            menuManager.getSelectedIndex()
