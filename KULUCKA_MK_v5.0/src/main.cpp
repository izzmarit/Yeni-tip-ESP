...
void handleValueAdjustment(JoystickDirection direction) {
    if (direction == JOYSTICK_PRESS) {
        ...
        // **DÜZELTME: PID ayarlarında MENU_PID menüsünde kal**
        if (prevState >= MENU_PID_KP && prevState <= MENU_PID_KD) {
            menuManager.setCurrentState(MENU_PID);
        }
        // Manuel kuluçka ayarlarında alt menüde kal
...
void loadSettingsFromStorage() {
    ...
    // **DÜZELTME: Motor zamanlama durumunu storage'dan yükle**
...
void saveSettingsToStorage() {
    ...
    // **DÜZELTME: Motor zamanlama durumunu da kaydet**
...
void handleWifiParameterUpdate(String param, String value) {
    ...
    // PID modu değişikliği
        ...
        if (mode >= 0 && mode <= 2) {
            pidController.setPIDMode((PIDMode)mode);
    ...
    // Alarm ayarları ve durum
        ...
        alarmManager.disableAlarm(!enabled);
        storage.queueSave();
    }
