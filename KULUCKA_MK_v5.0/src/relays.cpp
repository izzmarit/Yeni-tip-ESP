...
void Relays::loadMotorTimingFromStorage(Storage* storage) {
    ...
    // Sistem yeniden başlatıldıysa zamanı hesapla
    unsigned long currentMillis = millis();
    
    if (savedState == 1) { // RUNNING durumunda kapatılmış
        // Motor çalışırken sistem kapanmış, motoru kapat
        _motorState = false;
        _motorWaitStartTime = currentMillis;  // Doğru değişken ismi
        ...
        // Bekleme süresini restore et
        _motorWaitStartTime = currentMillis - savedElapsedTime;  // Doğru değişken ismi
        
        uint32_t remainingWait = (_waitTimeMinutes * 60000UL) - savedElapsedTime;  // Doğru değişken ismi
...
void Relays::saveMotorTimingToStorage(Storage* storage) {
    ...
    // Motor durumuna göre geçen süreyi hesapla
    if (_motorState) {
        // Motor çalışıyorsa
        elapsedTime = currentMillis - _motorRunStartTime;  // Doğru değişken ismi
        storage->setMotorTimingState(1); // RUNNING
    } else {
        // Motor beklemedeyse
        elapsedTime = currentMillis - _motorWaitStartTime;  // Doğru değişken ismi
