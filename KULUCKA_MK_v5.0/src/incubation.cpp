...
void Incubation::_initializeParameters() {
    ...
    _chickenParams.developmentHumidity = 55;
    _chickenParams.hatchingHumidity = 65;
...
uint8_t Incubation::getCurrentDay(DateTime currentTime) const {
    ...
    // DÜZELTME: Negatif zaman farkı kontrolü eklendi
    if (currentTime.unixtime() < _startTime.unixtime()) {
        Serial.println("UYARI: Mevcut zaman başlangıç zamanından küçük! Sistem saati kontrol edilmeli.");
        return 1; // En az 1. gün döndür
    }
    ...
    // DÜZELTME: Geçerli gün aralığı kontrolü
    if (daysPassed > 255) { // uint8_t overflow kontrolü
        Serial.println("UYARI: Gün sayısı çok büyük! Değer: " + String(daysPassed));
        return 255;
    }
