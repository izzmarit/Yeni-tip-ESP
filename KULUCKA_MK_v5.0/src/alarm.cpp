...
AlarmManager::AlarmManager() {
    ...
    _areAlarmsEnabled = true;  // Yeni: Varsayılan olarak alarmlar etkin
...
void AlarmManager::setAlarmsEnabled(bool enabled) {
    ...
    if (!enabled) {
        resetAlarm(); // Alarmlar kapatılırken mevcut alarmı sıfırla
    }
}
...
AlarmType AlarmManager::checkAlarms(float currentTemp, float targetTemp, 
                                   ...
                                   bool sensorsWorking) {
    // Alarmlar devre dışıysa veya eski disable sistemi aktifse
...
void AlarmManager::update() {
    if (_isAlarmActive && !_isAlarmDisabled && _areAlarmsEnabled) {
