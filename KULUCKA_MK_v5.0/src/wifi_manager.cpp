...
bool WiFiManager::switchToStationMode() {
    ...
    if (_storage != nullptr) {
        _storage->saveStateNow(); // Mode değişiminden önce tüm verileri kaydet
    }
    ...
    bool connected = beginStation(_stationSSID, _stationPassword);
    esp_task_wdt_reset();
    ...
    if (_storage != nullptr) {
        _storage->setWifiMode(WIFI_CONN_MODE_STATION);
        _storage->saveStateNow(); // Mode değişiminden sonra da kaydet
    }
...
String WiFiManager::_getStatusJson() {
    ...
    // PID durum bilgileri (external referans)
    ...
    doc["mdnsName"] = _mdnsName + ".local";
    doc["staticIP"] = _useStaticIP ? _staticIP.toString() : "DHCP";
...
void WiFiManager::_setupRoutes() {
    ...
    // Mevcut WiFi ayarlarını al
    _server->on("/api/wifi/settings", HTTP_GET, [this]() {
    StaticJsonDocument<400> doc;
    doc["mode"] = (_storage != nullptr) ? 
                  (int)_storage->getWifiMode() : 
                  (int)WIFI_CONN_MODE_AP;
    doc["apSSID"] = AP_SSID;
    doc["stationSSID"] = _stationSSID;
    doc["hasPassword"] = _stationPassword.length() > 0;
...
_server->on("/api/ping", HTTP_GET, [this]() {
    ...
    // WiFi mod değiştirme endpoint'i ekleyin:
_server->on("/api/wifi/mode", HTTP_POST, [this]() {
    ...
    if (doc.containsKey("mode")) {
        ...
        } else if (mode == "Station" && _stationSSID.length() > 0) {
        ...
        if (success) {
            ...
            _server->send(500, "application/json", _createErrorResponse("Mode değiştirme başarısız"));
        }
    } else {
    ...
    _server->on("/api/network/info", HTTP_GET, [this]() {
...
void WiFiManager::_handleSetTemperature() {
    ...
    if (doc.containsKey("targetTemp")) {
        ...
        // ÖNEMLİ: WiFiManager'ın kendi değişkenini de güncelle
        _targetTemp = targetTemp;
