...
bool WiFiManager::beginStation(const String& ssid, const String& password) {
    ...
    // Bağlantı başarısız olsa bile credential'ları kaydet
...
bool WiFiManager::switchToStationMode() {
    ...
    // KRITIK: Mevcut sistem durumunu koru
    float savedTemp = _currentTemp;
    float savedHumid = _currentHumid;
    float savedTargetTemp = _targetTemp;
    float savedTargetHumid = _targetHumid;
    bool savedHeaterState = _heaterState;
    bool savedHumidifierState = _humidifierState;
    bool savedMotorState = _motorState;
    int savedPidMode = _pidMode;
    ...
    // Sistem durumunu restore et
    _currentTemp = savedTemp;
    _currentHumid = savedHumid;
    _targetTemp = savedTargetTemp;
    _targetHumid = savedTargetHumid;
    _heaterState = savedHeaterState;
    _humidifierState = savedHumidifierState;
    _motorState = savedMotorState;
    _pidMode = savedPidMode;
...
void WiFiManager::_handleMotorTest() {
    ...
    // Test süresi parametresini al (opsiyonel)
    uint32_t testDuration = _motorRunTime; // Varsayılan olarak ayarlı çalışma süresini kullan
    if (doc.containsKey("duration")) {
        testDuration = doc["duration"];
        // Güvenlik için maksimum süre kontrolü
        testDuration = min(testDuration, (uint32_t)60); // Maksimum 60 saniye
    }
    
    // Motor test komutunu işle
