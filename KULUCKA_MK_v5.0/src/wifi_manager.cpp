...
// Global OTA Manager nesnesine erişim
...
WiFiManager::WiFiManager() {
    ...
    _lastServerCheck = 0;
    _serverUptime = 0;
    _serverRestartCount = 0;
    
    // Bellek koruma - YENİ EKLENECEK
    _lastFreeHeap = ESP.getFreeHeap();
    _minFreeHeap = _lastFreeHeap;
    _memoryProtectionActive = false;
...
bool WiFiManager::begin() {
    ...
    Serial.println("Station SSID: " + _stationSSID);
    Serial.println("Station şifre uzunluğu: " + String(_stationPassword.length()));
    
    // WiFi modülünü tamamen sıfırla
    ...
    if (mode == WIFI_CONN_MODE_STATION) {
        if (_stationSSID.length() > 0) {
            ...
            Serial.println("Bağlanılacak SSID: " + _stationSSID);
            
            // Station modunda başlat
            bool result = beginStation(_stationSSID, _stationPassword);
            
            if (result) {
                Serial.println("WiFi Manager: Station modunda başarıyla başlatıldı");
                return true;
            } else {
                Serial.println("WiFi Manager: Station bağlantısı başarısız, station modunda kalıyor");
            ...
            // SSID yoksa AP moduna geç ve kaydet
...
void WiFiManager::handleRequests() {
    ...
    if (_isServerRunning && _server) {
        ...
        // Periyodik bellek kontrolü - YENİ EKLENECEK
        unsigned long currentMillis = millis();
        if (currentMillis - _lastServerCheck > 10000) {  // Her 10 saniyede bir
            _lastServerCheck = currentMillis;
            
            uint32_t currentHeap = ESP.getFreeHeap();
            
            // Bellek durumu loglama
            if (currentHeap < _lastFreeHeap - 5000) {  // 5KB'dan fazla azalma
                Serial.println("WiFi: Bellek azalması tespit edildi: " + 
                             String(_lastFreeHeap) + " -> " + String(currentHeap));
            }
            
            // Kritik bellek seviyesi kontrolü
            if (currentHeap < 30000 && !_memoryProtectionActive) {
                Serial.println("WiFi: KRİTİK - Düşük bellek! " + String(currentHeap) + " bytes");
                _memoryProtectionActive = true;
                
                // Acil bellek temizleme
                esp_task_wdt_reset();
                delay(100);
                
                // Garbage collection tetikleme
                if (currentHeap < 25000) {
                    Serial.println("WiFi: Acil bellek temizleme başlatılıyor...");
                    _emergencyMemoryCleanup();
                }
            } else if (currentHeap > 50000 && _memoryProtectionActive) {
                _memoryProtectionActive = false;
                Serial.println("WiFi: Bellek normale döndü");
            }
            
            _lastFreeHeap = currentHeap;
            if (currentHeap < _minFreeHeap) {
                _minFreeHeap = currentHeap;
            }
        }
    }
}
...
void WiFiManager::processAppData(String jsonData) {
    // DÜZELTME: Buffer boyutunu artır
    StaticJsonDocument<2048> doc; // 1024'ten 2048'e çıkarıldı
    ...
    if (error) {
        ...
        if (error == DeserializationError::NoMemory) {
            Serial.println("WiFi Manager: JSON buffer yetersiz!");
        }
        return;
    }
...
_server->on("/api/wifi/mode", HTTP_POST, [this]() {
    ...
    if (doc.containsKey("mode")) {
        ...
        // Android uygulaması "station" veya "ap" string değerleri gönderiyor
        bool success = false;
        if (mode == "station") {
        ...
        // Sistem durumunu koru
        JsonObject systemState = response.createNestedObject("systemState");
        systemState["temperature"] = _currentTemp;
        systemState["humidity"] = _currentHumid;
        systemState["targetTemp"] = _targetTemp;
        systemState["targetHumid"] = _targetHumid;
...
_server->on("/api/system/verify", HTTP_GET, [this]() {
    ...
    // PID durum sorgulama endpoint'i
    _server->on("/api/pid/status", HTTP_GET, [this]() {
        _handlePidStatus();
    });

    // Motor durum sorgulama endpoint'i
    _server->on("/api/motor/status", HTTP_GET, [this]() {
        _handleMotorStatus();
    });
...
// OTA güncelleme endpoint'leri
_server->on("/api/ota/info", HTTP_GET, [this]() {
    extern OTAManager otaManager;
    
    StaticJsonDocument<512> doc;
    doc["currentVersion"] = otaManager.getFirmwareVersion();
    doc["buildDate"] = otaManager.getBuildDate();
    doc["updateAvailable"] = false;
    doc["updateInProgress"] = otaManager.isUpdateInProgress();
...
_server->on("/api/ota/update", HTTP_POST, 
    [this]() {
        ...
        if (otaManager.getState() == OTA_SUCCESS) {
            _server->send(200, "application/json", 
                         "{\"status\":\"success\",\"message\":\"Update completed successfully\"}");
        } else {
            _server->send(400, "application/json", 
                         "{\"status\":\"error\",\"message\":\"" + otaManager.getErrorMessage() + "\"}");
        }
    },
    [this]() {
        // Upload devam ederken çağrılır
        HTTPUpload& upload = _server->upload();
        extern OTAManager otaManager;
        
        if (upload.status == UPLOAD_FILE_START) {
            Serial.printf("OTA: Upload başladı: %s, Boyut: %d\n", 
                         upload.filename.c_str(), upload.totalSize);
            
            String md5 = "";
            if (_server->hasHeader("X-MD5")) {
                md5 = _server->header("X-MD5");
            }
            
            if (!otaManager.startUpdate(upload.totalSize, md5)) {
                _server->send(400, "text/plain", otaManager.getErrorMessage());
                return;
            }
            ...
            _server->send(400, "text/plain", "Update aborted");
        }
    }
);
...
_server->on("/api/ota/progress", HTTP_GET, [this]() {
    ...
    StaticJsonDocument<256> doc;
    doc["state"] = otaManager.getState();
    doc["progress"] = otaManager.getProgress();
    doc["totalSize"] = otaManager.getTotalSize();
    doc["writtenSize"] = otaManager.getWrittenSize();
...
void WiFiManager::_handleMotorTest() {
    ...
    uint32_t testDuration = 14; // Varsayılan değer
    if (doc.containsKey("duration")) {
        testDuration = doc["duration"];
        testDuration = constrain(testDuration, 1, 60); // 1-60 saniye arası
    }
    
    Serial.println("Web API Motor test isteği: " + String(testDuration) + " saniye");
...
void WiFiManager::_handleSetPidParameters() {
    ...
    if (kp >= PID_KP_MIN && kp <= PID_KP_MAX) {
        _processParameterUpdate("pidKp", String(kp)); // Doğru
        _pidKp = kp;
...
void WiFiManager::_safeServerRestart() {
    ...
    // DÜZELTME: Server'ı güvenli şekilde temizle
    if (_server != nullptr) {
        if (_isServerRunning) {
            _server->stop();
            _isServerRunning = false;
...
void WiFiManager::startServer() {
    ...
    uint32_t currentHeap = ESP.getFreeHeap();
    if (currentHeap < 40000) {  // 40KB altında ise server başlatma
        Serial.println("WiFi: Yetersiz bellek, server başlatılamıyor: " + String(currentHeap) + " bytes");
        _memoryProtectionActive = true;
        return;
    }
    
    // Server zaten varsa ve çalışıyorsa tekrar oluşturma
    if (_server != nullptr && _serverInitialized) {
        // Mevcut server'ı yeniden kullan
        _server->begin();
        _isServerRunning = true;
        _serverUptime = millis();
        Serial.println("WiFi: Mevcut server yeniden başlatıldı");
        return;
    }
    
    // İlk kez server oluşturma
    if (_server == nullptr) {
        _server = new WebServer(WIFI_PORT);
        if (_server == nullptr) {
            Serial.println("WiFi: Server oluşturma hatası - bellek yetersiz!");
            return;
        }
    ...
    _isServerRunning = true;
    _serverUptime = millis();
    _lastServerCheck = millis();
    
    // Bellek durumunu kaydet
    _lastFreeHeap = ESP.getFreeHeap();
    if (_lastFreeHeap < _minFreeHeap) {
        _minFreeHeap = _lastFreeHeap;
    }
    
    Serial.println("WiFi: Web sunucu başlatıldı: " + getIPAddress() + ":" + String(WIFI_PORT));
    Serial.println("Free heap: " + String(_lastFreeHeap) + " bytes (Min: " + String(_minFreeHeap) + ")");
}
...
void WiFiManager::stopServer() {
    ...
    // Server'ı durdur ama nesneyi SAKLA
    _server->stop();
    _isServerRunning = false;
    
    // Server uptime hesapla
    unsigned long uptime = millis() - _serverUptime;
    Serial.println("WiFi: Server uptime: " + String(uptime / 1000) + " saniye");
    
    // Bellek durumunu kontrol et
    uint32_t currentHeap = ESP.getFreeHeap();
    Serial.println("WiFi: Server durduruldu - Free heap: " + String(currentHeap) + " bytes");
    
    // NOT: Server nesnesini SİLME, tekrar kullanılacak
}

// PID durum handler'ı
void WiFiManager::_handlePidStatus() {
    // PID kontrolcüsüne erişim için extern referans
    ...
    doc["pidModeString"] = modeStr;
    
    // PID parametreleri
    JsonObject parameters = doc.createNestedObject("parameters");
    parameters["kp"] = pidController.getKp();
    parameters["ki"] = pidController.getKi();
    parameters["kd"] = pidController.getKd();
    
    // PID kontrol değerleri
    JsonObject control = doc.createNestedObject("control");
    control["setpoint"] = pidController.getSetpoint();
    control["currentInput"] = sensors.readTemperature();
    control["error"] = pidController.getError();
    control["output"] = pidController.getOutput();
    control["outputActive"] = pidController.isOutputActive();
    
    // PID durumu
    JsonObject status = doc.createNestedObject("status");
    status["active"] = pidController.isManualModeActive();
    status["autoTuneEnabled"] = pidController.isAutoTuneEnabled();
    status["autoTuneFinished"] = pidController.isAutoTuneFinished();
    status["autoTuneProgress"] = pidController.getAutoTuneProgress();
    
    // Isıtıcı durumu
    doc["heaterState"] = _heaterState;
    doc["heaterActive"] = pidController.isOutputActive();
...
// Motor durum handler'ı
void WiFiManager::_handleMotorStatus() {
    // Relays modülüne erişim için extern referans
    ...
    // Motor temel durumu
    doc["motorState"] = _motorState;
    doc["motorActive"] = relays.getMotorState();
    
    // Motor zamanlama bilgileri
    JsonObject timing = doc.createNestedObject("timing");
    timing["waitTime"] = _motorWaitTime;
    timing["runTime"] = _motorRunTime;
    timing["waitTimeLeft"] = relays.getMotorWaitTimeLeft();
    timing["runTimeLeft"] = relays.getMotorRunTimeLeft();
    
    // Motor döngü durumu
    JsonObject cycle = doc.createNestedObject("cycle");
    if (_motorState) {
        cycle["status"] = "RUNNING";
        cycle["phase"] = "RUN";
    } else {
        if (relays.getMotorWaitTimeLeft() > 0) {
    ...
    test["available"] = !_motorState;  // Motor çalışmıyorsa test yapılabilir
    test["lastTestTime"] = 0;  // Bu veri şu anda saklanmıyor
    
    // Motor istatistikleri (opsiyonel - storage'da saklanması gerekir)
    JsonObject statistics = doc.createNestedObject("statistics");
    statistics["totalRunCount"] = 0;  // Bu veri şu anda saklanmıyor
    statistics["totalRunTime"] = 0;   // Bu veri şu anda saklanmıyor
    statistics["lastRunDuration"] = _motorRunTime;
...
void WiFiManager::_emergencyMemoryCleanup() {
    Serial.println("WiFi: Acil bellek temizleme başladı");
    
    // JSON ve response buffer'ları temizle (varsa)
    if (_jsonBuffer != nullptr) {
        free(_jsonBuffer);
        _jsonBuffer = nullptr;
    }
    ...
    // Sistem bellek durumunu zorla temizle
    delay(100);
    
    uint32_t afterCleanup = ESP.getFreeHeap();
    Serial.println("WiFi: Bellek temizleme tamamlandı. Kazanılan: " + 
                   String(afterCleanup - _lastFreeHeap) + " bytes");
}

void WiFiManager::_startDiscoveryService() {
    if (!_discoveryUDP.begin(8266)) {
        Serial.println("Discovery UDP başlatılamadı!");
        return;
    }
    _discoveryEnabled = true;
    Serial.println("Discovery servisi port 8266'da başlatıldı");
}

void WiFiManager::_handleDiscoveryPackets() {
    if (!_discoveryEnabled) return;
    
    int packetSize = _discoveryUDP.parsePacket();
    if (packetSize > 0) {
        char buffer[64];
        int len = _discoveryUDP.read(buffer, 63);
        buffer[len] = '\0';
        
        if (strcmp(buffer, "KULUCKA_DISCOVERY") == 0) {
            IPAddress remoteIP = _discoveryUDP.remoteIP();
            uint16_t remotePort = _discoveryUDP.remotePort();
            
            // Rate limiting - saniyede bir yanıt
            unsigned long now = millis();
            if (now - _lastDiscoveryResponse > 1000) {
                _sendDiscoveryResponse(remoteIP, remotePort);
                _lastDiscoveryResponse = now;
            }
        }
    }
}

void WiFiManager::_sendDiscoveryResponse(IPAddress remoteIP, uint16_t remotePort) {
    String response = "KULUCKA_DEVICE:";
    response += WiFi.getMode() == WIFI_AP ? "AP" : "STATION";
    response += ":";
    response += String(WIFI_PORT);
    
    _discoveryUDP.beginPacket(remoteIP, remotePort);
    _discoveryUDP.print(response);
    _discoveryUDP.endPacket();
    
    Serial.println("Discovery yanıtı gönderildi: " + remoteIP.toString());
}

void WiFiManager::_setupWiFiEventHandlers() {
    WiFi.onEvent([this](WiFiEvent_t event, WiFiEventInfo_t info) {
        switch(event) {
            ...
            case SYSTEM_EVENT_AP_STADISCONNECTED:
                Serial.println("İstemci ayrıldı");
                _updateConnectedClients();
                break;
                
            case SYSTEM_EVENT_STA_DISCONNECTED:
                Serial.println("WiFi bağlantısı koptu");
                _handleStationDisconnect();
                break;
                
            case SYSTEM_EVENT_STA_GOT_IP:
                Serial.println("IP alındı: " + WiFi.localIP().toString());
                _handleStationConnect();
                break;
        }
    });
}

void WiFiManager::_setupMDNS() {
    if (!MDNS.begin("kulucka")) {
        Serial.println("mDNS başlatma hatası!");
        return;
    }
    ...
    // Özel servis bilgileri ekle
    MDNS.addServiceTxt("http", "tcp", "device", "KULUCKA_MK_v5");
    MDNS.addServiceTxt("http", "tcp", "version", "5.0");
    MDNS.addServiceTxt("http", "tcp", "mode", WiFi.getMode() == WIFI_AP ? "AP" : "Station");
    
    Serial.println("mDNS servisi başlatıldı: kulucka.local");
}
