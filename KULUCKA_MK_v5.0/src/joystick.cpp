...
void Joystick::update() {
    ...
    // Hali hazırda bir buton basma olayı algılanmışsa çıkış yap
    if (_currentDirection == JOYSTICK_PRESS) {
        return;
    }
    ...
    // Eğer yeni bir yön algılandıysa ve yeterli süre geçtiyse
    if (newDirection != JOYSTICK_NONE) {
        if ((newDirection != _lastDirection || 
            (currentTime - _lastActionTime) > 300)) { // 300ms tekrarlama gecikmesi
        ...
        // böylece aynı yöne tekrar hareket edildiğinde hemen algılanabilir
        if (abs(_xPosition - _xCenter) < (_threshold / 2) && 
