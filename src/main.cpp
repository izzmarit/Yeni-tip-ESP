...
void handleJoystick() {
    ...
    if (direction != JOYSTICK_NONE) {
        ...
        if (currentState == MENU_NONE) {
            ...
            // Ana ekranda diğer yönler hiçbir şey yapmaz
            return;
        }
        ...
        feedWatchdogIfNeeded();
        // JOYSTICK RESET MEKANİZMASI KALDIRILDI!
    }
}
...
void handleMenuActions(JoystickDirection direction) {
    ...
    if (direction == JOYSTICK_RIGHT) {
        switch (currentState) {
            case MENU_TEMPERATURE:
                menuManager.showValueAdjustScreen("Sicaklik", incubation.getTargetTemperature(), "C", 20.0, 40.0, 0.1);
                break;
                
            case MENU_HUMIDITY:
                menuManager.showValueAdjustScreen("Nem", incubation.getTargetHumidity(), "%", 30, 90, 1);
                break;
            ...
            case MENU_MOTOR_WAIT:
                menuManager.showValueAdjustScreen("Bekleme", relays.getMotorWaitTimeLeft(), "dk", 1, 240, 1);
                break;
                
            case MENU_MOTOR_RUN:
                menuManager.showValueAdjustScreen("Calisma", relays.getMotorRunTimeLeft(), "sn", 1, 60, 1);
                break;
                
            case MENU_SET_TIME:
                {
                    DateTime now = rtc.getCurrentDateTime();
                    menuManager.showValueAdjustScreen("Saat", now.hour() * 100 + now.minute(), "", 0, 2359, 1);
                }
                break;
            ...
            case MENU_SET_DATE:
                {
                    DateTime now = rtc.getCurrentDateTime();
                    menuManager.showValueAdjustScreen("Tarih", now.day() * 1000000 + now.month() * 10000 + now.year(), "", 1010000, 31122099, 1);
                }
                break;
    ...
    if (direction == JOYSTICK_PRESS) {
        switch (currentState) {
            case MENU_INCUBATION_TYPE:
                {
                    ...
                    // Manuel seçildi ise (indeks 3)
                    if (selectedType == 3) {
                        // Manuel kuluçka alt menüsüne geç
                        menuManager.setCurrentState(MENU_MANUAL_INCUBATION);
                    } else if (selectedType < 3) {
                        // Diğer kuluçka tiplerini seç ve kaydet
            ...
            // Değer ayarlama ekranında kaydetme
...
void handleValueAdjustment(JoystickDirection direction) {
    if (direction == JOYSTICK_PRESS) {
        ...
        switch (prevState) {
            ...
            case MENU_MOTOR_WAIT:
                relays.updateMotorTiming(millis(), (uint32_t)value, relays.getMotorRunTimeLeft());
                storage.setMotorWaitTime((uint32_t)value);
                storage.queueSave();
                break;
                
            case MENU_MOTOR_RUN:
                relays.updateMotorTiming(millis(), relays.getMotorWaitTimeLeft(), (uint32_t)value);
            ...
            case MENU_SET_DATE:
                {
                    ...
                    if (day >= 1 && day <= 31 && month >= 1 && month <= 12 && year >= 2023 && year <= 2099) {
                        DateTime now = rtc.getCurrentDateTime();
